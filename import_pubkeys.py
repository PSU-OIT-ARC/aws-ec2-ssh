...
class Database:
    ...
    def createDatabase(self):
        sql = "CREATE TABLE IF NOT EXISTS sshkeys (username VARCHAR(20) NOT NULL, pubkey VARCHAR(512) NOT NULL, updated INTEGER DEFAULT 0);"
        self.cursor.execute(sql)

    def updateKey(self, username, pubkey):
        timestamp = int(time.time())

        sql = "SELECT updated FROM sshkeys WHERE username = ? and pubkey = ?"
        self.cursor.execute(sql, [username, pubkey])

        results = self.cursor.fetchall()
        if not results:
            sql = "INSERT INTO sshkeys VALUES (?,?,?)"
            self.cursor.execute(sql, [username, pubkey, timestamp])
        else:
            sql = "UPDATE sshkeys SET updated=? WHERE username=? AND pubkey=?"
            self.cursor.execute(sql, [timestamp, username, pubkey])
